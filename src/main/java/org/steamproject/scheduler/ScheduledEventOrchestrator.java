...
public class ScheduledEventOrchestrator {
    ...
    private void runInitializationPhase() {
        // D'abord créer les éditeurs (pas d'événement Kafka, juste en mémoire locale)
        System.out.println("🏢 Création de " + INIT_PUBLISHERS_COUNT + " éditeurs...");
        for (int i = 0; i < INIT_PUBLISHERS_COUNT; i++) {
            PublisherInfo pub = generator.generatePublisher();
            dataStore.addPublisher(pub);
        }
        System.out.println("   ✅ " + dataStore.getPublisherCount() + " éditeurs créés\n");
