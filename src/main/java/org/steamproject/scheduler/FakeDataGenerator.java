...
public class FakeDataGenerator {
    ...
    /**
     * Génère un événement de publication de patch avec un type aléatoire.
     */
    public PatchPublishedEvent generatePatch(GameInfo game) {
        // Type aléatoire par défaut
        PatchType[] types = PatchType.values();
        return generatePatch(game, types[random.nextInt(types.length)]);
    }
    
    /**
     * Génère un événement de publication de patch avec un type spécifique.
     * 
     * @param game Le jeu concerné
     * @param mainType Le type principal du patch (FIX, ADD, OPTIMIZATION)
     */
    public PatchPublishedEvent generatePatch(GameInfo game, PatchType mainType) {
        String oldVersion = game.currentVersion();
        String newVersion = incrementVersionForType(oldVersion, mainType);

        List<Change> changes = new ArrayList<>();
        int numChanges = 2 + random.nextInt(4); // 2 à 5 changements
        
        // Le premier changement est toujours du type principal
        changes.add(Change.newBuilder()
                .setType(mainType)
                .setDescription(getDescriptionForType(mainType))
                .build());

        // Les autres changements sont du même type ou mixtes
        for (int i = 1; i < numChanges; i++) {
            // 70% de chance d'être du même type
            PatchType changeType = random.nextDouble() < 0.7 ? mainType : 
                PatchType.values()[random.nextInt(PatchType.values().length)];
            changes.add(Change.newBuilder()
                    .setType(changeType)
                    .setDescription(getDescriptionForType(changeType))
                    .build());
        }

        // Taille du patch selon le type
        long sizeInMB = getSizeForType(mainType);
        
        // Changelog selon le type
        String changeLog = getChangeLogForType(mainType);
    ...
    /**
     * Incrémente la version selon le type de patch (SemVer).
     * - FIX: x.y.Z+1 (patch)
     * - OPTIMIZATION: x.Y+1.0 (minor)  
     * - ADD: X+1.0.0 (major)
     */
    private String incrementVersionForType(String version, PatchType type) {
        try {
    ...
    /**
     * Retourne une description appropriée pour le type de changement.
     */
    private String getDescriptionForType(PatchType type) {
        switch (type) {
            case FIX:
                return randomFrom(List.of(
                    ...
                    "Fix des problèmes de sauvegarde",
                    "Correction de bugs de progression",
                    "Résolution de crashes multijoueur",
                    "Fix de problèmes de compatibilité"
                ));
            case OPTIMIZATION:
                return randomFrom(List.of(
                    ...
                    "Optimisation des shaders graphiques",
                    "Amélioration de la stabilité serveur",
            ...
            case ADD:
                return randomFrom(List.of(
                    ...
                    "Nouveau mode de jeu disponible",
                    "Ajout de personnages jouables",
                    "Nouvelles armes et équipements",
                    "Nouvelle zone explorable",
                    "Ajout du mode Photo",
    ...
    /**
     * Retourne une taille appropriée en MB selon le type de patch.
     */
    private long getSizeForType(PatchType type) {
        switch (type) {
            case FIX:
                return 50 + random.nextInt(200);      // 50-250 MB
            case OPTIMIZATION:
                return 100 + random.nextInt(400);     // 100-500 MB
            case ADD:
                return 500 + random.nextInt(1500);    // 500 MB - 2 GB
            default:
    ...
    /**
     * Retourne un changelog approprié selon le type de patch.
     */
    private String getChangeLogForType(PatchType type) {
        switch (type) {
            case FIX:
                return "Patch de correction - Amélioration de la stabilité et correction de bugs signalés par la communauté.";
            case OPTIMIZATION:
                return "Patch d'optimisation - Amélioration des performances et de l'expérience de jeu.";
            case ADD:
                return "Mise à jour majeure - Nouveau contenu et fonctionnalités demandées par les joueurs!";
            default:
    ...
    /**
     * Génère un événement de vote sur une évaluation.
     * Un joueur vote sur l'utilité d'une évaluation faite par un autre joueur.
     */
    public ReviewVotedEvent generateReviewVote(String reviewId, String voterPlayerId) {
        return ReviewVotedEvent.newBuilder()
                .setEventId(UUID.randomUUID().toString())
                .setReviewId(reviewId)
                .setVoterPlayerId(voterPlayerId)
                .setIsHelpful(random.nextDouble() < 0.7) // 70% de votes positifs
                .setVoteAction(VoteAction.CAST)
                .setTimestamp(Instant.now().toEpochMilli())
                .build();
    }
